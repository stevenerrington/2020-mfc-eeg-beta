...
clear betaValues
for session = 1:29
    betaValues.saccade.noncanc{session} = nanmean(betaConvolved.saccade{session}(executiveBeh.ttx.NC{session},:));
    betaValues.saccade.nostop{session} = nanmean(betaConvolved.saccade{session}(executiveBeh.ttx.GO{session},:));
end
